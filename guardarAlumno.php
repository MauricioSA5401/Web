...
if (
    ...
    $stmt = $conn->prepare("INSERT Alumno VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, (SELECT u.matricula FROM Usuario u WHERE matricula = $matricula))");

    if ($stmt === false) {
        die('Error en la preparación de la consulta SQL: ' . htmlspecialchars($conn->error));
    }
    
    $stmt->bind_param("sssssssssss", $matricula, $nombre, $ape1, $ape2, $tel1, $correo, $direccion, $ciudad, $tel2, $carrera, $semestre);
    ...
    if ($stmt->execute()) {
        ...
        echo '<script>
            ...
            window.location.href = "../form-Alumno.php";
